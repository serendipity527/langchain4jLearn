# 文档处理系统设计模式重构说明

## 重构概述

本次重构使用**策略模式（Strategy Pattern）**和**工厂模式（Factory Pattern）**对整个文档处理流程进行了全面优化：

1. **文档加载器重构**: 将加载逻辑从 `DocumentService` 中解耦
2. **文档解析器重构**: 将解析逻辑从加载器中分离，实现智能选择
3. **文档分割器重构**: 提供多种分割策略，支持动态选择（新增）

> **📚 详细文档**: 
> - 加载器设计说明: 本文档
> - 解析器设计说明: `文档解析器设计模式说明.md`
> - 分割器设计说明: `文档分割器设计模式说明.md`

---

## 设计模式应用

### 1. 策略模式（Strategy Pattern）

**目的**: 定义一系列算法（加载策略），把它们一个个封装起来，并且使它们可以相互替换。

**实现**:
- **策略接口**: `DocumentLoaderStrategy`
- **具体策略**:
  - `FileSystemDocumentLoaderStrategy` - 文件系统加载策略
  - `UrlDocumentLoaderStrategy` - URL加载策略
  - `ClasspathDocumentLoaderStrategy` - 类路径加载策略

### 2. 工厂模式（Factory Pattern）

**目的**: 提供一个创建对象的接口，让工厂类决定实例化哪一个策略类。

**实现**:
- **工厂类**: `DocumentLoaderFactory`
- **功能**: 根据 `DocumentLoaderType` 枚举返回对应的加载器策略

---

## 架构设计

```
┌─────────────────────────────────────────────────────┐
│                  RagController                       │
│  (调用DocumentService处理文档摄取请求)                │
└────────────────────┬────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────┐
│               DocumentService                        │
│  • 负责文档摄取的主要业务逻辑                         │
│  • 委托加载逻辑给DocumentLoaderFactory                │
└────────────────────┬────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────┐
│           DocumentLoaderFactory                      │
│  • 根据DocumentLoaderType选择对应的加载器策略         │
│  • Spring自动注入所有策略实现                         │
└────────────────────┬────────────────────────────────┘
                     │
         ┌───────────┼───────────┐
         ▼           ▼           ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│FileSystem    │ │Url           │ │Classpath     │
│Loader        │ │Loader        │ │Loader        │
│Strategy      │ │Strategy      │ │Strategy      │
└──────────────┘ └──────────────┘ └──────────────┘
         │           │           │
         └───────────┴───────────┘
                     │
         实现 DocumentLoaderStrategy 接口
```

---

## 核心类说明

### 1. DocumentLoaderStrategy（策略接口）

```java
public interface DocumentLoaderStrategy {
    // 加载单个文档
    Document loadDocument(String sourcePath);
    
    // 批量加载文档
    List<Document> loadDocuments(String sourcePath);
    
    // 获取加载器类型
    DocumentLoaderType getLoaderType();
}
```

**职责**: 定义文档加载的统一行为规范

---

### 2. DocumentLoaderType（枚举）

```java
public enum DocumentLoaderType {
    FILE_SYSTEM,  // 文件系统加载器
    URL,          // URL加载器
    CLASSPATH     // 类路径加载器
}
```

**职责**: 定义支持的加载器类型

---

### 3. 具体策略实现

#### FileSystemDocumentLoaderStrategy
- **功能**: 从本地文件系统加载文档
- **特性**: 
  - 自动识别PDF文件并使用 `ApachePdfBoxDocumentParser`
  - 其他文件使用 `TextDocumentParser`
  - 支持单文件和目录批量加载

#### UrlDocumentLoaderStrategy
- **功能**: 从URL加载文档
- **特性**: 
  - 使用 langchain4j 的 `UrlDocumentLoader`
  - 自动处理网络请求

#### ClasspathDocumentLoaderStrategy
- **功能**: 从类路径（resources目录）加载文档
- **特性**: 
  - 从项目内嵌资源加载
  - 适合预打包的文档

---

### 4. DocumentLoaderFactory（工厂类）

```java
@Component
@RequiredArgsConstructor
public class DocumentLoaderFactory {
    // Spring自动注入所有策略实现
    private final List<DocumentLoaderStrategy> loaderStrategies;
    
    // 根据类型获取加载器
    public DocumentLoaderStrategy getLoader(DocumentLoaderType loaderType) {
        return loaderStrategies.stream()
                .filter(strategy -> strategy.getLoaderType() == loaderType)
                .findFirst()
                .orElseThrow(...);
    }
}
```

**职责**: 
- 管理所有加载器策略实例
- 根据类型返回对应的策略
- 利用Spring的依赖注入自动收集所有策略实现

---

### 5. DocumentService（重构后）

**重构前**:
- 包含大量的文档加载逻辑
- 每种加载方式都有独立的实现
- 代码冗余，难以维护

**重构后**:
```java
public void ingestDocumentFromFile(String filePath) {
    DocumentLoaderStrategy loader = loaderFactory.getLoader(DocumentLoaderType.FILE_SYSTEM);
    Document document = loader.loadDocument(filePath);
    ingestDocuments(List.of(document));
}
```

**优势**:
- 代码简洁，每个方法只有3-4行
- 加载逻辑委托给专门的策略类
- 符合单一职责原则

---

## 重构优势

### 1. **开闭原则（Open-Closed Principle）**
- ✅ 对扩展开放：添加新的加载器只需实现 `DocumentLoaderStrategy` 接口
- ✅ 对修改关闭：无需修改现有代码

### 2. **单一职责原则（Single Responsibility Principle）**
- ✅ `DocumentService` 专注于文档摄取业务逻辑
- ✅ 各策略类专注于各自的加载逻辑
- ✅ 工厂类专注于策略的管理和创建

### 3. **依赖倒置原则（Dependency Inversion Principle）**
- ✅ `DocumentService` 依赖于抽象（`DocumentLoaderStrategy`）而非具体实现
- ✅ 通过工厂获取策略，降低耦合

### 4. **可维护性**
- ✅ 代码结构清晰，职责分明
- ✅ 每个类的代码量减少，易于理解
- ✅ 修改某个加载器不影响其他部分

### 5. **可扩展性**
- ✅ 添加新的加载器类型只需：
  1. 创建新的策略实现类
  2. 在枚举中添加新类型
  3. Spring自动注入，无需修改工厂类

### 6. **可测试性**
- ✅ 每个策略可以独立测试
- ✅ 可以轻松 mock 策略进行单元测试
- ✅ 工厂类的测试更加简单

---

## 如何扩展新的加载器

假设要添加一个 **数据库加载器**：

### 步骤1: 添加枚举类型
```java
public enum DocumentLoaderType {
    FILE_SYSTEM,
    URL,
    CLASSPATH,
    DATABASE  // 新增
}
```

### 步骤2: 创建策略实现
```java
@Component
public class DatabaseDocumentLoaderStrategy implements DocumentLoaderStrategy {
    
    @Autowired
    private DocumentRepository documentRepository;
    
    @Override
    public Document loadDocument(String sourcePath) {
        // 从数据库加载逻辑
        return documentRepository.findById(sourcePath)...;
    }
    
    @Override
    public List<Document> loadDocuments(String sourcePath) {
        // 批量加载逻辑
        return documentRepository.findAll()...;
    }
    
    @Override
    public DocumentLoaderType getLoaderType() {
        return DocumentLoaderType.DATABASE;
    }
}
```

### 步骤3: 完成！
- ✅ Spring 自动注入新策略到工厂
- ✅ 无需修改 `DocumentService`
- ✅ 无需修改 `DocumentLoaderFactory`
- ✅ 可以立即使用

---

## 使用示例

### 在代码中使用

```java
@Autowired
private DocumentService documentService;

// 方式1: 使用具体方法
documentService.ingestDocumentFromFile("C:/docs/example.pdf");
documentService.ingestDocumentFromUrl("https://example.com/doc.txt");
documentService.ingestDocumentFromClasspath("docs/readme.md");

// 方式2: 使用动态方法（推荐）
documentService.ingestDocumentByLoaderType(
    DocumentLoaderType.FILE_SYSTEM, 
    "C:/docs/example.pdf"
);
```

### 通过 API 使用

```bash
# 动态加载接口
curl -X POST http://localhost:8080/api/rag/ingest-dynamic \
  -H "Content-Type: application/json" \
  -d '{
    "loaderType": "FILE_SYSTEM",
    "sourcePath": "C:/documents/example.pdf"
  }'
```

---

## 项目结构

```
src/main/java/org/example/ragtest/
├── loader/                          # 文档加载器包（新增）
│   ├── DocumentLoaderStrategy.java  # 策略接口
│   ├── DocumentLoaderType.java      # 类型枚举
│   ├── DocumentLoaderFactory.java   # 工厂类
│   └── impl/                        # 策略实现
│       ├── FileSystemDocumentLoaderStrategy.java
│       ├── UrlDocumentLoaderStrategy.java
│       └── ClasspathDocumentLoaderStrategy.java
├── service/
│   └── DocumentService.java         # 重构后的服务类
└── controller/
    └── RagController.java           # 控制器
```

---

## 总结

通过引入策略模式和工厂模式，我们实现了：

1. **代码更简洁**: `DocumentService` 从 200+ 行缩减到 150 行
2. **职责更清晰**: 每个类都有明确的单一职责
3. **扩展更容易**: 添加新加载器无需修改现有代码
4. **测试更简单**: 每个组件可以独立测试
5. **维护更方便**: 修改某个加载器不影响其他部分

这是一个优秀的**面向对象设计**实践案例，遵循了 SOLID 原则，使代码更加专业和工业级。
